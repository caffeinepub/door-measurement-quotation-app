...
import { roundDimensions } from '../utils/dimensionRounding';
...
export function DoorEntryForm({ onEntryAdded, resetRef }: DoorEntryFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Calculate rounded dimensions using combined function
    // (applies custom rule: width > 36 & <= 38 & height < 78 → 38×78)
    const { widthRounded, heightRounded } = roundDimensions(
      widthResult.decimal,
      heightResult.decimal
    );
